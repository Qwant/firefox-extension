...
exports.init = function(firstUse) {
    ...
    mainPopup = panels.Panel({
        width: 320, 
        height: 640,
        contentURL: './main/mainpopup.html',
        contentScriptFile: './main/mainpopup.js'
    });
    mainPopup.panelToShow = '';

    mainPopup.on("show", function() {
        let toShow = mainPopup.panelToShow;
        if (!toShow && showWelcome) {
            toShow = 'welcome';
        }
        let options = {
            panelToShow: toShow,
            ...
            canAddNote : boardnotes.isButtonActivated(),
            canAddFavorite : bookmarks.isButtonActivated()
        };
    ...
    mainPopup.port.on("about", function() {
        tabs.open({
            url: self.data.url('https://www.qwant.com/extension/firefox/about')
        });
        mainPopup.hide();
    });
...
exports.showPopup = function(panelToShow) {
    mainPopup.panelToShow = panelToShow;
    mainButton.click();
}
