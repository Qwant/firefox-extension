	...
	var qwantForFirefox = function(worker) {
		...
		worker.port.on('search', function(data) {
			...
			if (Core.sPrefs.prefs.save_last === true) {
				Core.sStorage.storage.qwantQuery = data.query.query;
				Interface.qwantPopup.port.emit('storage', Core.sStorage.storage.qwantQuery);
			}

			Core.request.Request({
				url: 'https://api.qwant.com/api/search/smart?q=' + data.query + '&locale=' + qwtEmbed.getLocale() + '&client=firefox-ext',
				anonymous: true,
				onComplete: function(response) {
					worker.port.emit('data', response.json.data.result);
				}
			}).get();
		});
