    ...
    return callApi('/account/login', {email, password}, 'post')
        .then(function(response) {
            ...
            if ('favorites' in response.data && Array.isArray(response.data.favorites)) {
                return bookmarks.setBookmarks(response.data.favorites);
            }
            return true;
        });
}
    ...
    return callBoardApi('/board/getFlatList')
        .then(function(response) {
            if (response.status != 'success') {
                ...
                if (response.error == 208) {
                    throw new Error(_('boardnote.error.board.permission.denied'));
                }
        ...
        .then(function(response) {
            if (response.status != 'success') {
                ...
                if (response.error == 201) {
                    throw new Error(_('boardnote.error.board.not.found'));
                }
                if (response.error == 202) {
                    throw new Error(_('boardnote.error.board.private'));
                }
                if (response.error == 208) {
                    throw new Error(_('boardnote.error.board.permission.denied'));
                }
            ...
            if (response.status != 'success') {
                if (response.error != 401) { // 401 = link already exists
                    ...
                    if (response.error == 402) {
                        throw new Error(_('bookmark.error.favorite.notcrawlable'));
                    }
                    throw new Error(_('bookmark.error.create.favorite'));
                }
                else {
                    throw new Error(_('bookmark.error.create.already.exists'));
                }
                return false;
            }
            return true;
...
exports.getFavorites = function() {
    return callUserApi('/account/favorite')
        .then(function(response) {
            if (response.status != 'success') {
                console.debug("Error on /account/favorite" + JSON.stringify(response));
                throw new Error(_('bookmark.error.get.favorites'));
            }
            return response.data;
        });
}
