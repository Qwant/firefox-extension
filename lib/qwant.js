...
var api = function (route, params) {
    return new Promise(function (resolve, reject) {
        ...
        if (route.method === "POST") {
            ...
            if (config.DEBUG) {
                console.log("Params sent to API are : " + JSON.stringify(params));
            }
        }
        else {
        ...
        if (config.DEBUG) {
            uri += (route.method === "POST")?"?" + config.XDEBUG:config.XDEBUG;
        }
        ...
        if (config.DEBUG) {
            console.log("Calling the API at route : " + uri + " with method " + route.method);
        }
        client.send(data);
        ...
        client.onload = function () {
            if (this.status >= 200 && this.status < 300) {
                if (config.DEBUG) {
                    console.log("Response from the API : " + JSON.stringify(JSON.parse(this.response)));
                }
